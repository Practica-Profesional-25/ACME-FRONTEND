...
export function PurchaseSummary({
  ...
  const [paymentMethod, setPaymentMethod] = useState<
    "efectivo" | "tarjeta" | null
  >(null);
  const [cashAmount, setCashAmount] = useState("");
  const [posStatus, setPosStatus] = useState<"ready" | "processed">("ready");

  const subtotalAmount = saleData.products.reduce(
    (sum, product) => sum + product.subtotal * product.quantity,
    0
  );
  const taxAmount = saleData.products.reduce(
    (sum, product) => sum + product.tax * product.quantity,
    0
  );
  const totalAmount =
    saleData.products.reduce((sum, product) => sum + product.total, 0) +
    taxAmount;
  const change = cashAmount
    ? Math.max(
        0,
        Number.parseFloat(
          (Number.parseFloat(cashAmount) - totalAmount).toFixed(2)
        )
      )
    : 0;
              ...
              <div className="mt-1">
                <div className="flex items-center gap-2">
                  <Badge variant="secondary">
                    {saleData.customer?.type || "N/A"}
                  </Badge>
                  <span className="font-medium">
                    {saleData.customer?.name || "No seleccionado"}
                  </span>
                </div>
              ...
              {paymentMethod === "efectivo" && (
                <Card className="ml-6">
                  <CardContent className="pt-4">
                    ...
                    {Number.parseFloat(cashAmount) < totalAmount &&
                      cashAmount && (
                        <p className="text-sm text-destructive mt-2">
                          El monto entregado debe ser mayor o igual al total de
                          la venta
                        </p>
                      )}
                  </CardContent>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="mt-2 flex items-center gap-4">
                          <Badge
                            variant={
                              posStatus === "ready" ? "secondary" : "default"
                            }
          ...
          <Card className="border-secondary">
            <CardContent className="pt-4">
              <div className="flex items-center gap-2 text-secondary">
                ...
                <span className="font-medium">
                  Pago en efectivo validado correctamente
                </span>
              </div>
            </CardContent>
          </Card>
